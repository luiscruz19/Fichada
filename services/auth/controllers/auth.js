...
export async function login(req, res) {
    ...
    try {
        ...
        if (!validate_password) {
            await registerFailedAttempt(email, ip);
            logLogin({ email, user_id: user.id, is_admin: user.is_admin, status: 'wrong_password', ...meta });
            return res.status(401).json(errorMessage({ message: messages.error.login.error }));
        }
        if (!user.verified) {
            await registerFailedAttempt(email, ip);
            logLogin({ email, user_id: user.id, is_admin: user.is_admin, status: 'not_verified', ...meta });
        ...
        if (data.totp_enabled) {
            ...
            logLogin({ email, user_id: data.id, is_admin: data.is_admin, status: 'pending_2fa', ...meta });
        ...
        logLogin({ email, user_id: data.id, is_admin: data.is_admin, status: 'success', ...meta });
