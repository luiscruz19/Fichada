...
if (isProduction) {
    const required = ['AUTH_BASIC_USER', 'AUTH_BASIC_PW', 'MAILER_USER'];
    ...
    if (!process.env.MAILER_SMTP_HOST && !process.env.MAILER_SMPT_HOST) {
        console.error('[config] Missing required env var in production: MAILER_SMTP_HOST');
        process.exit(1);
    }
}
